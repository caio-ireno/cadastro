...
const getAll = async (page=1, filter=''): Promise<SorveteComTotalCount | Error> => {
  try{
    const urlRelativa=`/mais-populares?_page=${page}&_limit=${Environment.LIMITE_LINHAS}&nomeSorvete_like=${filter}`;
...
const getById = async (id:number): Promise<ListaSorveteProps | Error> => {
  try{
    const {data} = await Api.get(`/mais-populares/${id}`);
...
const create = async (dados:Omit<ListaSorveteProps, 'id'>): Promise<number | Error> => {
  try{
    const {data} = await Api.post<ListaSorveteProps>('/mais-populares', dados); //Dessa forma eu consigo dizer qqual dado esta retornando
...
const updateById = async (id:number, dados:ListaSorveteProps): Promise<void | Error> => {
  try{
    await Api.put(`/mais-populares/${id}`, dados); 
...
const deleteById = async (id:number): Promise<void | Error> => {
  try{
    await Api.delete<ListaSorveteProps>(`/mais-populares/${id}`); 
...
export const SorveteServiceMaisPopulares ={
  getAll,
  getById,
  create,
  updateById,
