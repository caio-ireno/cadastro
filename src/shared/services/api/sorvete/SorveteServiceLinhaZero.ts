...
const getAll = async (page=1, filter=''): Promise<SorveteComTotalCount | Error> => {
  try{
    const urlRelativa=`/linha-zero?_page=${page}&_limit=${Environment.LIMITE_LINHAS}&nomeSorvete_like=${filter}`;
...
const getById = async (id:number): Promise<ListaSorveteProps | Error> => {
  try{
    const {data} = await Api.get(`/linha-zero/${id}`);
...
const create = async (dados:Omit<ListaSorveteProps, 'id'>): Promise<number | Error> => {
  try{
    const {data} = await Api.post<ListaSorveteProps>('/linha-zero', dados); //Dessa forma eu consigo dizer qqual dado esta retornando
...
const updateById = async (id:number, dados:ListaSorveteProps): Promise<void | Error> => {
  try{
    await Api.put(`/linha-zero/${id}`, dados); 
...
const deleteById = async (id:number): Promise<void | Error> => {
  try{
    await Api.delete<ListaSorveteProps>(`/linha-zero/${id}`); 
...
export const SorveteServiceLinhaZero ={
  getAll,
  getById,
  create,
  updateById,
