...
const getAll = async (page=1, filter=''): Promise<SorveteComTotalCount | Error> => {
  try{
    const urlRelativa=`/copao?_page=${page}&_limit=${Environment.LIMITE_LINHAS}&nomeSorvete_like=${filter}`;
...
const getById = async (id:number): Promise<ListaSorveteProps | Error> => {
  try{
    const {data} = await Api.get(`/copao/${id}`);
...
const create = async (dados:Omit<ListaSorveteProps, 'id'>): Promise<number | Error> => {
  try{
    const {data} = await Api.post<ListaSorveteProps>('/copao', dados); //Dessa forma eu consigo dizer qqual dado esta retornando
...
const deleteById = async (id:number): Promise<void | Error> => {
  try{
    await Api.delete<ListaSorveteProps>(`/copao/${id}`); 
...
export const SorveteServiceCopao ={
  getAll,
  getById,
  create,
  updateById,
