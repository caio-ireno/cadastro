...
const getAll = async (page=1, filter=''): Promise<SorveteComTotalCount | Error> => {
  try{
    const urlRelativa=`/sorveteEspecial?_page=${page}&_limit=${Environment.LIMITE_LINHAS}&nomeSorvete_like=${filter}`;
...
const getById = async (id:number): Promise<ListaSorveteProps | Error> => {
  try{
    const {data} = await Api.get(`/sorveteEspecial/${id}`);
...
const create = async (dados:Omit<ListaSorveteProps, 'id'>): Promise<number | Error> => {
  try{
    const {data} = await Api.post<ListaSorveteProps>('/sorveteEspecial', dados); //Dessa forma eu consigo dizer qqual dado esta retornando
...
const updateById = async (id:number, dados:ListaSorveteProps): Promise<void | Error> => {
  try{
    await Api.put(`/sorveteEspecial/${id}`, dados); 
...
const deleteById = async (id:number): Promise<void | Error> => {
  try{
    await Api.delete<ListaSorveteProps>(`/sorveteEspecial/${id}`); 
...
export const SorveteServiceEspecial ={
  getAll,
  getById,
  create,
  updateById,
